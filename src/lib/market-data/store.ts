...
export async function recallSnapshot<T>(
  ...
  try {
    const row = await prisma.marketDataSnapshot.findFirst({
      where: { kind, fetchedAt: { gt: new Date(Date.now() - MAX_FALLBACK_AGE_MS) } },
 ...
 */
export async function pruneSnapshots(kind: MarketDataKind, keep = 5): Promise<void> {
  try {
    ...
    await prisma.marketDataSnapshot.deleteMany({
      where: { kind, id: { notIn: survivors.map((s) => s.id) } },
    });
  } catch (error) {
